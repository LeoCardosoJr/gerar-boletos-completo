...
module.exports = class BoletoStringify {
    ...
    static enderecoAvalista({ logradouro, bairro, cidade, estadoUF, cep }) {
      return Boleto.Endereco.novoEndereco()
        .comLogradouro(logradouro)
        .comBairro(bairro)
        .comCidade(cidade)
    ...
    static createAvalista({ endereco, nome, registroNacional }) {
      const enderecoAvalista = this.enderecoAvalista(endereco);
      return Boleto.Avalista.novoAvalista()
        .comNome(nome)
        .comRegistroNacional(registroNacional)
        .comEndereco(enderecoAvalista);
    }
