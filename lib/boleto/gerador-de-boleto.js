	...
	GeradorDeBoleto.prototype.gerarPDF = function(args) {
		return new Promise((resolve)=> {
			...
			boletos.forEach(function escreveOsDadosDoBoleto(boleto, indice) {
				...
				const avalista = boleto.getAvalista();
				...
				const TEM_AVALISTA = !!avalista?.getIdentificacao();
				...
				let linhaComSacadorAvalista = TEM_AVALISTA ? 106.6 : 56.6;
				var linha212 = linha211 + linhaComSacadorAvalista;
				...
				if (TEM_AVALISTA) {
					pdf.font('negrito')
						.fontSize(args.tamanhoDaFonteDoTitulo)
						.text('Sacador/Avalista', args.ajusteX + 30, args.ajusteY + tituloDaSetimaLinha + 170, {
							lineBreak: false,
							width: 294,
							align: 'left'
						});
					...
					pdf.font('normal')
						.fontSize(args.tamanhoDaFonte)
						.text(avalista.getIdentificacao(), args.ajusteX + 30, args.ajusteY + tituloDaSetimaLinha + 177, {
							lineBreak: false,
							width: 535,
							align: 'left'
						});

					var enderecoDoAvalista = avalista.getEndereco();
					if (enderecoDoAvalista) {
						var espacamento = args.tamanhoDaFonte;

						if (enderecoDoAvalista.getPrimeiraLinha()) {
							pdf.font('normal')
								.fontSize(args.tamanhoDaFonte)
								.text(enderecoDoAvalista.getPrimeiraLinha(), args.ajusteX + 30, args.ajusteY + tituloDaSetimaLinha + 177 + espacamento, {
									lineBreak: false,
									width: 535,
									align: 'left'
								});
						...
						if (enderecoDoAvalista.getSegundaLinha()) {
							pdf.font('normal')
								.fontSize(args.tamanhoDaFonte)
								.text(enderecoDoAvalista.getSegundaLinha(), args.ajusteX + 30, args.ajusteY + tituloDaSetimaLinha + 177 + (espacamento), {
									lineBreak: false,
									width: 535,
									align: 'left'
								});
				...
				let posicAutenticacaoMecanica = TEM_AVALISTA ? 226.5 : 175;
				pdf.font('negrito')
					.fontSize(args.tamanhoDaFonteDoTitulo)
					.text('Autenticação Mecânica', args.ajusteX + 420, args.ajusteY + tituloDaSetimaLinha + posicAutenticacaoMecanica, {
						lineBreak: false,
						width: 294,
						align: 'left'
					});
